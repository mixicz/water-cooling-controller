...
{
    if (eeprom.fan_user_config[fan].is_pump) {
        if (up)
            return eeprom.config.pump_ramp_up_step_time;
        else
            return eeprom.config.pump_ramp_down_step_time;
    } else {
        if (up)
            return eeprom.config.fan_ramp_up_step_time;
        else
            return eeprom.config.fan_ramp_down_step_time;
    }
}
...
#ifdef DEBUG_FAN
        twr_log_debug("fan_ramp_step(): FAN=%i, ignoring request: enabled=%d, pwm control=%d, calibration in process=%d", fan_runtime[fan].enabled, fan_runtime[fan].pwm_control, fan_runtime[fan].calibration_in_progress);
#endif
        return;
    }
...
void fan_set_speed(uint8_t fan, float speed)
{
    ...
    if (fan_runtime[fan].enabled == false || fan_runtime[fan].pwm_control == false || fan_runtime[fan].calibration_in_progress == true) {
#ifdef DEBUG_FAN
        twr_log_debug("fan_set_speed(): FAN=%i, ignoring request: enabled=%d, pwm control=%d, calibration in process=%d", fan_runtime[fan].enabled, fan_runtime[fan].pwm_control, fan_runtime[fan].calibration_in_progress);
#endif
        return;
    }
    if (speed < eeprom.fan_user_config[fan].min_speed)
        speed = eeprom.fan_user_config[fan].min_speed;
    if (speed > eeprom.fan_user_config[fan].max_speed)
        speed = eeprom.fan_user_config[fan].max_speed;
    if (eeprom.fan_calibration[fan].calibrated == true) {
        float index = (float)(FAN_CALIBRATION_STEPS-1) * speed;
        float weight = index - (int)index;
        if ((int)index >= FAN_CALIBRATION_STEPS-1)
            fan_runtime[fan].pwm_target = eeprom.fan_calibration[fan].points[FAN_CALIBRATION_STEPS-1];
        else if ((int)index < 0)
            fan_runtime[fan].pwm_target = eeprom.fan_calibration[fan].points[0];
        else
            // linear interpolation between 2 points in calibration data (PWM values)
            fan_runtime[fan].pwm_target = (uint16_t)((float)eeprom.fan_calibration[fan].points[(int)index] * (1.0 - weight) + (float)eeprom.fan_calibration[fan].points[(int)index+1] * weight);
        // cap PWM value to minimum PWM value as detected by calibration process
        if (fan_runtime[fan].pwm_target < eeprom.fan_calibration[fan].min_pwm)
            fan_runtime[fan].pwm_target = eeprom.fan_calibration[fan].min_pwm;
    } else {
...
void fan_compute_calibration(uint8_t fan)
{
    ...
    eeprom.fan_user_config[fan].max_speed = 1.0;
    eeprom.fan_user_config[fan].min_speed = 0.2;
...
#endif
        eeprom.fan_calibration[fan].calibrated = false;
        eeprom.fan_calibration[fan].pwm_capable = false;   
        eeprom.fan_calibration[fan].present = false;
        return;
    }
    eeprom.fan_calibration[fan].present = true;
    ...
    eeprom.fan_calibration[fan].pwm_capable = true;   
    ...
    if ((float)(pwm20_max_rpm - pwm20_min_rpm) / (float)pwm20_max_rpm > 0.6)
    {
        ...
        if (eeprom.fan_user_config[fan].is_pump) {
#ifdef DEBUG_FAN_CALIBRATION
            twr_log_debug("fan_compute_calibration(): FAN %i: configured as pump, setting minimal speed to 20%%", fan);
#endif
            eeprom.fan_user_config[fan].min_speed = 0.2;
            eeprom.fan_calibration[fan].min_pwm = FAN_PWM_MIN;
        }
    ...
    for (uint8_t i = 0; i < FAN_CALIBRATION_STEPS; i++)
    {
        uint8_t idx = FAN_CALIBRATION_STEPS-1-i;
        ...
        if (fan_calibration_measure[fan][i].rpm > 0) {
            rpm_deviation[idx] = (float)(fan_calibration_measure[fan][i].rpm - rpm) / (float)rpm;
        } else {
            rpm_deviation[idx] = 0.0;
        }
#endif
        int idx_low = -1;
        int idx_high = -1;
        ...
        if (idx_low > -1 && idx_high > -1) {
            ...
            if (pwm > FAN_PWM_MAX) {
                pwm = FAN_PWM_MAX;
#ifdef DEBUG_FAN_CALIBRATION
                twr_log_warning("fan_compute_calibration(): FAN %i: PWM value overflow, speed=%.2f, pwm=%i, rpm=%i, pwm_low=%i, pwm_high=%i, rpm_low=%i, rpm_high=%i", 
                    fan, speed, pwm, rpm, pwm_low, pwm_high, rpm_low, rpm_high);
#endif
            }
            eeprom.fan_calibration[fan].points[idx] = pwm;
        } else {
            // Unable to compute this calibration point, fallback to assume linear PWM/RPM dependency
            eeprom.fan_calibration[fan].points[idx] = (uint16_t)((float)FAN_PWM_MAX * speed);
        }
        if (eeprom.fan_calibration[fan].points[idx] < eeprom.fan_calibration[fan].min_pwm)
            eeprom.fan_calibration[fan].points[idx] = eeprom.fan_calibration[fan].min_pwm;
    }
#ifdef DEBUG_FAN_CALIBRATION
    ...
    for (uint8_t i = 0; i < FAN_CALIBRATION_STEPS; i++) {
        ...
        float delta;
        if (expected_pwm > 0)
            delta = (float)(eeprom.fan_calibration[fan].points[i]-expected_pwm) / (float)expected_pwm;
        else
            delta = 0.0;
        twr_log_debug("fan_compute_calibration(): FAN %i computed data: speed=%.2f, pwm=%i, rpm=%i, correction=%1.1f%%, RPM deviation=%1.1f%%", 
            fan, (float)i / (float)(FAN_CALIBRATION_STEPS-1), eeprom.fan_calibration[fan].points[i], (uint16_t)((float)max_rpm * ((float)i / (float)(FAN_CALIBRATION_STEPS-1))), delta * 100.0, rpm_deviation[i] * 100.0);
    }
#endif
}
...
twr_scheduler_task_id_t fan_calibration_task_id[MAX_FANS] = {0};
...
void fan_calibration_step(void * ptr_fan)
{
    ...
    static uint16_t tolerance[MAX_FANS] = {0, 0, 0, 0, 0};
    uint8_t fan = *(uint8_t*)ptr_fan;
    ...
    if (step[fan] > 0) {
        ...
        // use slowly increasing tolerance to prevent infinite loop
        if (abs((int)rpm - (int)last_rpm[fan]) > tolerance[fan]) {
#ifdef DEBUG_FAN_CALIBRATION
    ...
    step[fan]++;
    if (step[fan] > FAN_CALIBRATION_STEPS) {
        // end of calibration - set FAN speed to previous target value
        twr_pwm_set(fan_config[fan].pwm_port, fan_runtime[fan].pwm_target);
        fan_runtime[fan].calibration_in_progress = false;
        step[fan] = 0;
        twr_scheduler_unregister(twr_scheduler_get_current_task_id());
        twr_log_info("fan_calibration_step(): FAN %i calibration done", fan);
#ifdef DEBUG_FAN_CALIBRATION
        for (uint8_t i = 0; i < FAN_CALIBRATION_STEPS; i++)
...
{
    if (fan_runtime[fan].calibration_in_progress == true)
        return;
    twr_log_info("fan_calibration_start(): FAN %i calibration start", fan);
    eeprom.fan_calibration[fan].calibrated = false;
    fan_runtime[fan].calibration_in_progress = true;
    twr_pwm_set(fan_config[fan].pwm_port, FAN_PWM_MAX);
    fan_calibration_task_id[fan] = twr_scheduler_register(fan_calibration_step, &fan_list[fan], twr_tick_get() + FAN_CALIBRATION_TIME_INIT);
    char topic[64];
    sprintf(topic, "fan/calibration/%i/state", fan);
...
void fan_calibration_stop(uint8_t fan) {
    if (fan_runtime[fan].calibration_in_progress == false)
        return;
    twr_log_warning("fan_calibration_stop(): FAN %i calibration stop", fan);
    fan_runtime[fan].calibration_in_progress = false;
    twr_pwm_set(fan_config[fan].pwm_port, fan_runtime[fan].pwm_target);
    twr_scheduler_unregister(fan_calibration_task_id[fan]);
    fan_calibration_task_id[fan] = 0;
    char topic[64];
    sprintf(topic, "fan/calibration/%i/state", fan);
    twr_radio_pub_string(topic, "stop");
    twr_led_set_mode(&led, TWR_LED_MODE_OFF);
}